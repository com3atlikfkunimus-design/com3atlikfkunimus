...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function fetchSupabaseAthletes() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const merged = data.map((db, idx) => {
            ...
            // Hybrid local storage Sesi 1 results lookup
            let localSesi1 = {};
            try {
            ...
            // Hybrid local storage Sesi 2 results lookup
            let localSesi2 = {};
            try {
              if (typeof window !== 'undefined') {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-8">
        ...
        {/* ── Section 1: Premium Statistic Metric Cards ── */}
        ...
        {/* ── Section 2: Chart.js Side-by-Side Comparative Graphs ── */}
        ...
        {/* ── Section 3: Clinical Trials Database Table ── */}
        ...
        {/* ── Section 4: Create New Athlete Registration Form ── */}
        <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.015)]">
          <div className="mb-6">
            <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Registrasi Naracoba / Atlet Baru</h2>
            <p className="text-[9px] text-slate-400 mt-0.5">Daftarkan profil atlet baru langsung dari dashboard penelitian untuk memulai pengujian.</p>
          </div>

          <form onSubmit={handleAddAthlete} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div className="space-y-1">
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Nama Atlet</label>
              <input
                type="text"
                placeholder="Contoh: Ahmad Fauzi"
                value={newAthlete.name}
                onChange={(e) => setNewAthlete(prev => ({ ...prev, name: e.target.value }))}
                className="w-full bg-transparent border-b border-slate-200 focus:border-[#2563eb] py-2 text-xs text-[#0f172a] placeholder-slate-300 outline-none rounded-none transition-all"
              />
            </div>

            <div className="space-y-1">
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Umur</label>
              <input
                type="number"
                placeholder="Umur"
                value={newAthlete.age}
                onChange={(e) => setNewAthlete(prev => ({ ...prev, age: e.target.value }))}
                className="w-full bg-transparent border-b border-slate-200 focus:border-[#2563eb] py-2 text-xs text-[#0f172a] placeholder-slate-300 outline-none rounded-none transition-all"
              />
            </div>

            <div className="space-y-1">
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Program Studi</label>
              <select
                value={newAthlete.prodi}
                onChange={(e) => setNewAthlete(prev => ({ ...prev, prodi: e.target.value }))}
                className="w-full bg-transparent border-b border-slate-200 focus:border-[#2563eb] py-2 text-xs text-[#0f172a] outline-none rounded-none transition-all cursor-pointer"
              >
            ...
            <div className="space-y-1">
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Berat (kg) / Tinggi (cm)</label>
              <div className="flex gap-2">
                <input
                  type="number"
                  placeholder="BB (kg)"
                  value={newAthlete.weight}
                  onChange={(e) => setNewAthlete(prev => ({ ...prev, weight: e.target.value }))}
                  className="w-1/2 bg-transparent border-b border-slate-200 focus:border-[#2563eb] py-2 text-xs text-[#0f172a] placeholder-slate-300 outline-none rounded-none transition-all"
                />
                <input
                  type="number"
                  placeholder="TB (cm)"
                  value={newAthlete.height}
                  onChange={(e) => setNewAthlete(prev => ({ ...prev, height: e.target.value }))}
                  className="w-1/2 bg-transparent border-b border-slate-200 focus:border-[#2563eb] py-2 text-xs text-[#0f172a] placeholder-slate-300 outline-none rounded-none transition-all"
                />
              </div>
              {newAthleteBmi && (
                <span className="text-[8px] text-slate-400 font-semibold block mt-1">
                  BMI: {newAthleteBmi.bmi} ({newAthleteBmi.category})
                </span>
              )}
            </div>
            ...
            <div className="w-full">
              <button
                ...
                className="w-full py-3 bg-[#2563eb] hover:bg-[#1d4ed8] active:bg-[#1e40af] text-white rounded font-bold text-[9px] uppercase tracking-wider transition-all cursor-pointer text-center"
              >
                {isSubmitting ? 'Mendaftarkan...' : 'Daftarkan Atlet'}
              </button>
            </div>
          </form>
        </div>

        {/* ── Section 5: Create New Researcher Account Form ── */}
        <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.015)]">
          <div className="mb-6">
        ...
        {/* ── Section 6: Panel Konfigurasi Asesmen & Aturan Klinis ── */}
        {/* ── Section 6: Panel Konfigurasi Asesmen & Aturan Klinis ── */}
        <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.015)]">
